...
task("deposit:eth", "Deposit ETH into FixedRateETHVault")
  .addParam("vault", "ETH vault address")
  .addParam("amount", "ETH amount in ether", undefined, undefined, true)
  .setAction(async (args, hre) => {
    const amount = args.amount ? hre.ethers.parseEther(args.amount) : hre.ethers.parseEther("1");
    const vault = await hre.ethers.getContractAt("FixedRateETHVault", args.vault);
    const tx = await vault.deposit({ value: amount });
    await tx.wait();
    console.log(`Deposited ${args.amount || "1"} ETH -> ${args.vault}`);
  });

task("deposit:erc4626", "Deposit ERC20 into FixedRateERC4626Vault")
